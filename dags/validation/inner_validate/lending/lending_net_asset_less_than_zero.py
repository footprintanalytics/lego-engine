...
from validation.inner_validate.query import lending_asset_sum, group_by_asset, lending_liquidation_collateral, \
    lending_liquidation_repay


class LendingNetAssetLessThanZero(BaseInnerValidate):
    # 忽略计算的资产
    ignore_asset = []
    tolerance = 0
    ...
    def get_self_data(self):
        ...
        liquidation_repay = lending_liquidation_repay(self.chain, self.project)
        liquidation_collateral = lending_liquidation_collateral(self.chain, self.project)
    ...
    def check_rule(self, values):
        ...
        result_code = 0
        result_message = '净存入大于净借出校验通过'

        success = []
        fail = []
        for asset, asset_info in data_map.items():
            print('asset_info {}:  {}'.format(asset, asset_info))
            s = pydash.get(asset_info, 'supply', 0)
            w = pydash.get(asset_info, 'withdraw', 0)
            b = pydash.get(asset_info, 'borrow', 0)
            r = pydash.get(asset_info, 'repay', 0)
            l_r = pydash.get(asset_info, 'liquidation_repay', 0)
            l_c = pydash.get(asset_info, 'liquidation_collateral', 0)
            net_asset = (s - w) - (b - r) + l_r - l_c
            if asset not in self.ignore_asset and net_asset < 0:
                fail.append({'asset_token': asset, 'net_asset': net_asset})
            else:
                success.append({'asset_token': asset, 'net_asset': net_asset})
