from common.common_pool_model3 import InvestPoolModel3
from utils.constant import BUSINESS_SECOND_TYPE, CHAIN
from utils.import_gsc_to_bigquery import import_gsc_to_bigquery_base,get_schema
from config import project_config
from validation.basic_validate.abnormal_number.lending_abnormal_number_validate_liquidation import \
    LendingAbnormalNumberValidateLiquidation
from validation.basic_validate.data_continuity.data_continuity_lending_volume_validate_liquidation import \
    DataContinuityLendingVolumeValidateLiquidation
from validation.external_validate.bitquery.bitquery_contract_trades_count_validate import BitqueryContractTradesCountValidate
import pydash
from datetime import datetime, timedelta
from validation.basic_validate.data_continuity import DataContinuityLendingVolumeValidate
from validation.basic_validate.address_format_validate import AddressFormatValidate
from validation.basic_validate.negative_number_validate import NegativeNumberValidate
from validation.basic_validate.abnormal_number.lending_abnormal_number_validate import LendingAbnormalNumberValidate
from common.pool_all_data_view_builder import AllDataViewBuilder

bucket_name = project_config.bigquery_bucket_name


class LendingBaseModel(InvestPoolModel3):
    chain: str
    task_name: str
    dataset_name_prefix: str
    project_id = 'footprint-etl'
    schema_name = 'lending'

    def build_classify_transaction_sql(self, source: str):
        """
        先用原生 SQL 实现分类，后面处理复杂逻辑要用 JS UDF
        :return:
        """
        if self.task_name.split('_')[-1] == BUSINESS_SECOND_TYPE['LIQUIDATION']:  # 清算有一套独立的逻辑
            return self._liquidation(source)
        return self._normal(source)

    def _liquidation(self, source: str):
        return """
        WITH source_table AS (
                {source}
            ),
            -- 连 tokoen 表
            transactions_token AS(
                SELECT
                s.type,
                s.project,
                s.version,
                s.protocol_id,
                d.protocol_slug,
                s.block_number,
                s.block_time,
                s.tx_hash,
                s.log_index,
                s.contract_address,
                -- 借款人
                s.borrower,
                t.symbol as token_collateral_symbol,
                s.token_collateral_address,
                s.token_collateral_amount_raw / POW(10, t.decimals) AS token_collateral_amount,
                s.token_collateral_amount_raw,
                
                -- 清算人
                s.liquidator,
                tr.symbol as repay_token_symbol,
                s.repay_token_address,
                s.repay_token_amount_raw / POW(10, tr.decimals) AS repay_token_amount,
                s.repay_token_amount_raw,
                
                d.chain AS chain,
                s.pool_id
                FROM source_table s
                LEFT JOIN `xed-project-237404.footprint_etl.{chain}erc20_tokens` t
                ON LOWER(s.token_collateral_address) = LOWER(t.contract_address)
                LEFT JOIN `xed-project-237404.footprint_etl.{chain}erc20_tokens` tr
                ON LOWER(s.repay_token_address) = LOWER(tr.contract_address)
                left join `xed-project-237404.footprint.defi_protocol_info` d
                on s.protocol_id = d.protocol_id
            )
            SELECT * FROM transactions_token
            """.format(source=source, chain='{}_'.format(self.chain) if self.chain != CHAIN['ETHEREUM'] else '')


    def _normal(self, source: str):
        return """
            WITH source_table AS (
                {source}
            ),
            -- 连 tokoen 表
            transactions_token AS(
                SELECT
                s.type,
                s.project,
                s.version,
                s.protocol_id,
                d.protocol_slug,
                s.block_number,
                s.block_time,
                s.tx_hash,
                s.log_index,
                s.contract_address,
                s.operator,
                t.symbol as token_symbol,
                s.token_address,
                token_amount_raw / POW(10, t.decimals) AS token_amount,
                token_amount_raw,
                d.chain AS chain,
                s.pool_id
                FROM source_table s
                LEFT JOIN `xed-project-237404.footprint_etl.{chain}erc20_tokens` t
                ON LOWER(s.token_address) = LOWER(t.contract_address)
                left join `xed-project-237404.footprint.defi_protocol_info` d
                on s.protocol_id = d.protocol_id
            )
            SELECT * FROM transactions_token
            """.format(source=source, chain='{}_'.format(self.chain) if self.chain != CHAIN['ETHEREUM'] else '')

    def do_import_gsc_to_bigquery(self):
        """
               重写 do_import_gsc_to_bigquery 方法，将data_base 定义成footprint_lending_etl
        """
        schema_name = 'lending_liquidation' if self.task_name.split('_')[-1] == BUSINESS_SECOND_TYPE[
            'LIQUIDATION'] else 'lending_transaction'
        uri = "gs://{bucket_name}/{gsc_folder}/*.csv".format(bucket_name=bucket_name, gsc_folder=self.task_name)
        import_gsc_to_bigquery_base(schema=get_schema(schema_name), uri=uri,
                                    bigquery_table_name=self.get_daily_table_name(),name=self.schema_name)


    def get_daily_table_name(self):
        return '{}.{}_lending_{}.{}'.format(self.project_id, self.chain, self.dataset_name_prefix, self.task_name)

    def bitquery_validate(self,date_str):
        protocol_info = self.get_defi_protocol_info()
        for info in protocol_info:
            BitqueryContractTradesCountValidate(cfg={
                'validate_date': date_str,
                'validate_table': [self.get_daily_table_name()],
                'chain': pydash.get(info, 'chain', 'ethereum').lower(),
                'project': pydash.get(info, 'project'),
                'protocol_id': int(pydash.get(info, 'protocol_id'))
            }).validate()

        # 基础校验
    def validate(self, validate_table: str=None,interval_days: int=1 ):
        if not validate_table:
            validate_table = self.get_daily_table_name()+"_all"
        AddressFormatValidate({
            "validate_field": [
                'borrower',
                'token_collateral_address',
                'repay_token_address'
            ] if self.task_name.split('_')[-1] == BUSINESS_SECOND_TYPE['LIQUIDATION'] else [
                'operator',
                'token_address'
            ],
            "validate_table": validate_table,
            "chain": self.chain,
            "project": self.project_name
        }).validate()
        # DataContinuityLendingVolumeValidate({
        #     "validate_table": [validate_table + '_all'],
        #     "chain": self.chain,
        #     "project": self.project_name,
        #     "validate_date": (datetime.now() + timedelta(days=-90)).strftime('%Y-%m-%d')
        # }).validate()

        NegativeNumberValidate({
            "validate_field": ['repay_token_amount','token_collateral_amount'] if self.task_name.split('_')[-1] == BUSINESS_SECOND_TYPE['LIQUIDATION'] else ['token_amount'],
            "validate_table": validate_table,
            "chain": "ethereum",
            "project": self.project_name
        }).validate()

        if self.task_name.split('_')[-1] == BUSINESS_SECOND_TYPE['LIQUIDATION']:
            LendingAbnormalNumberValidateLiquidation({
                "validate_table": validate_table,
                "chain": "ethereum",
                "project": self.project_name
            }).validate()
        else:
            LendingAbnormalNumberValidate({
                "validate_table": validate_table,
                "chain": "ethereum",
                "project": self.project_name
            }).validate()

        if self.task_name.split('_')[-1] == BUSINESS_SECOND_TYPE['LIQUIDATION']:
            DataContinuityLendingVolumeValidateLiquidation({
                "interval_days": interval_days,
                "validate_table": [validate_table],
                "chain": "ethereum",
                "project": self.project_name
            }).validate()
        else:
            DataContinuityLendingVolumeValidate({
                "interval_days":interval_days,
                "validate_table": [validate_table],
                "chain": "ethereum",
                "project": self.project_name
            }).validate()

    def external_validate(self,date_str):
        self.bitquery_validate(date_str)

    def create_all_data_view(self):
        AllDataViewBuilder.build_all_data_view(
            transactions_table=self.get_daily_table_name(),
            transactions_history_table=self.get_history_table_name(),
            history_date=self.history_date,
            date_column='block_time'
        )

    ## todo 暂时去掉校验
    def airflow_steps(self):
        return [
            self.do_start,
            self.parse_daily_data,
            self.etl.do_upload_csv_to_gsc,
            self.do_import_gsc_to_bigquery,
            # self.validate,
            self.do_end
        ]
