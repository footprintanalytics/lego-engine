...
from polygon_event_etl.parse.parse_logic import ref_regex, parse, create_dataset
...
def build_parse_dag(
    ...
    def create_parse_task(table_definition):
        ...
        def parse_task(ds, **kwargs):
            ...
            if _ds:
                ds = _ds
            print('polygon parse_task 本次执行获取的ds为', ds)
            parse(
                ...
                sqls_folder=os.path.join(PROJECT_PATH, 'resources/stages/parse/polygon_sqls'),
    ...
    parse_polygon_dag_start_task = PythonOperator(
        task_id='parse_start',
        python_callable=parse_polygon_start,
        provide_context=True,
        execution_timeout=timedelta(minutes=60),
        dag=dag
    )
    wait_for_polygon_load_dag_task = BashOperator(
        task_id='partition_done',
        bash_command='echo parse starting',
        dag=dag
    )

    parse_polygon_dag_start_task >> wait_for_polygon_load_dag_task
    ...
    for json_file in json_files:
        ...
        wait_for_polygon_load_dag_task >> task
    ...
    parse_polygon_dag_end_task = PythonOperator(
        task_id='parse_end',
        python_callable=parse_polygon_end,
        provide_context=True,
        execution_timeout=timedelta(minutes=60),
        dag=dag
    )
    ...
    checkpoint_task >> share_dataset_task >> parse_polygon_dag_end_task
...
def parse_polygon_start(**kwargs):
    dag = kwargs.get('dag')
    dag_id = dag.dag_id
    print('dag_id', dag_id)
    print('任务执行开始')
