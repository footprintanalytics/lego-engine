...
from bsc_event_etl.parse.parse_logic import ref_regex, parse, create_dataset
...
def build_parse_dag(
    ...
    SOURCE_DATASET_NAME = 'crypto_bsc'
    ...
    # This prevents failing all dags as they are constructed in a loop in bsc_parse_dag.py
    ...
    def create_parse_task(table_definition):
        ...
        def parse_task(ds, **kwargs):
            ...
            if _ds:
                ds = _ds
            print('bsc parse_task 本次执行获取的ds为', ds)
            parse(
                ...
                sqls_folder=os.path.join(PROJECT_PATH, 'resources/stages/parse/bsc_sqls'),
    ...
    wait_for_bsc_load_dag_task = BashOperator(
    ...
    for json_file in json_files:
        ...
        wait_for_bsc_load_dag_task >> task
    ...
    full_dataset_name = 'bsc_' + dataset_name
    ...
    if notification_emails and len(notification_emails) > 0 and send_success_email:
        send_email_task = EmailOperator(
            ...
            subject='BSC ETL Airflow Parse DAG Succeeded',
            html_content='BSC ETL Airflow Parse DAG Succeeded for {}'.format(dag_id),
            dag=dag
        )
