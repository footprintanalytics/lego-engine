...
FROM
	(
        SELECT remove.block_number                          AS block_number,
               ...
               '0x130966628846bfd36ff31a822705796e8cb8c18d' AS repay_token_address,
               repay.part                                   AS repay_token_amount_raw,
               pool_info.pool_id                            AS pool_id
        FROM `footprint-etl.avalanche_abracadabra.CauldronV2Multichain_event_LogRemoveCollateral` remove
                 LEFT JOIN
             `footprint-etl.avalanche_abracadabra.CauldronV2Multichain_event_LogRepay` repay
    ...
    LEFT JOIN `footprint-blockchain-etl.crypto_avalanche.transactions` tx
