SELECT  'MDEX'                    AS project,
        ...
        CAST(w.amount AS FLOAT64) AS asset_amount,
...
(
    ...
    FROM
    (
        SELECT * FROM `footprint-etl.bsc_mdex.BSCPool_event_Withdraw`
        UNION ALL
        SELECT * FROM `footprint-etl.bsc_mdex.BSCPool_event_EmergencyWithdraw`
    ) _w
    ...
    FROM
    (
        SELECT * FROM `footprint-etl.bsc_mdex.BoardRoom_event_Withdraw`
        UNION ALL
        SELECT * FROM `footprint-etl.bsc_mdex.BoardRoom_event_EmergencyWithdraw`
    ) _w
) w
