...
from (
    ...
    where amount != '0'
    union all
    select block_timestamp, block_number, transaction_hash, log_index, contract_address, user, amount from `footprint-etl.bsc_venus.XVSVault_event_ExecutedWithdrawal`
) a
left join (
    select * from `footprint-blockchain-etl.crypto_bsc.token_transfers`
    where Date(block_timestamp) {match_date_filter}
) b
on a.transaction_hash = b.transaction_hash and b.to_address = a.user and b.from_address = a.contract_address
