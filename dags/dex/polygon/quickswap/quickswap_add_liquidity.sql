...
-- token0
    ...
    FROM
      `footprint-etl.polygon_quickswap.UniswapV2Pair_event_Mint` event
    LEFT JOIN
      `footprint-etl.polygon_quickswap.UniswapV2Factory_event_PairCreated`  pair_created
    ON
      (event.contract_address = pair_created.pair)
    LEFT JOIN `xed-project-237404.footprint_etl.polygon_erc20_tokens` t ON t.contract_address = pair_created.token0
    LEFT JOIN
      `footprint-blockchain-etl.crypto_polygon.transactions` tx
    ON
...
union all
    ...
    ON
      (event.contract_address = pair_created.pair)
      LEFT JOIN `xed-project-237404.footprint_etl.polygon_erc20_tokens` t ON t.contract_address = pair_created.token1
    LEFT JOIN
      `footprint-blockchain-etl.crypto_polygon.transactions` tx
    ON
      tx.hash = event.transaction_hash AND DATE(tx.block_timestamp) {match_date_filter}
