    ...
    FROM
        (SELECT * FROM (
            SELECT
                ...
                contract_address,
                token_address,
                REPLACE(token_amount,'-','') as token_amount
            FROM (
              SELECT *
              FROM (
                SELECT
                ...
                WHERE
                  REGEXP_CONTAINS(b.deltas, r'-[1-9]\d*') = TRUE),
...
SELECT
     ...
     '1' as version,
     t.symbol AS token_symbol,
     SAFE_DIVIDE(CAST(a.tokenAmountOut AS FLOAT64), POW(10, t.decimals)) as token_amount,
     CAST(a.tokenAmountOut AS FLOAT64) AS token_amount_raw,
     a.tokenOut as token_address,
     a.contract_address AS exchange_address,
     a.transaction_hash AS tx_hash,
     DATETIME(a.block_timestamp) AS block_time,
     tx.from_address AS tx_from
    FROM
     blockchain-etl.ethereum_balancer.BPool_event_LOG_EXIT a
     LEFT JOIN xed-project-237404.footprint_etl.erc20_tokens t ON Lower(t.contract_address) = a.tokenOut
     LEFT JOIN `bigquery-public-data.crypto_ethereum.transactions` tx ON tx.hash = a.transaction_hash AND DATE(tx.block_timestamp) {match_date_filter}
     where DATE(a.block_timestamp) {match_date_filter}
)