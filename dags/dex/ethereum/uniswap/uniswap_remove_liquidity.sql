...
--FROM
--    `blockchain-etl.ethereum_uniswap.Uniswap_Dai_event_RemoveLiquidity` a
...
FROM
  `blockchain-etl.ethereum_uniswap.UniswapV2Pair_event_Burn` event
LEFT JOIN
...
SELECT
    ...
    b.to AS tx_from
FROM
    `footprint-etl.ethereum_uniswap.UniswapV3PositionsNFT_event_DecreaseLiquidity` a
left join (
...
on
    a.tokenId = b.tokenId
left join
    (select distinct contract_address, transaction_hash, amount0, amount1 from `footprint-etl.ethereum_uniswap.UniswapV3Pool_event_Burn` ) c
on
