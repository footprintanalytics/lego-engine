...
,reward_token
,concat(if(tokena.symbol is null or tokena.symbol='',t.token0,tokena.symbol ),'/',if(tokenb.symbol is null or tokenb.symbol='',t.token1,tokenb.symbol)) as name
,"" as description
from
(SELECT
pair as pool_id,
...
left join `xed-project-237404.footprint_etl.bsc_erc20_tokens` tokena
on Lower(tokena.contract_address) = Lower(t.token0)
left join `xed-project-237404.footprint_etl.bsc_erc20_tokens` tokenb
