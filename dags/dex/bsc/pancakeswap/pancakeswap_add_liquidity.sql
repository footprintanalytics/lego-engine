...
-- token0
    ...
    FROM
      `footprint-etl.bsc_pancakeswap.PancakePair_event_Mint` event
    LEFT JOIN
      `footprint-etl.bsc_pancakeswap.UniswapV2Pair_event_PairCreated` pair_created
    ON
      (event.contract_address = pair_created.pair)
    LEFT JOIN `xed-project-237404.footprint_etl.bsc_erc20_tokens` t ON t.contract_address = pair_created.token0
    LEFT JOIN
...
union all
    ...
    ON
      (event.contract_address = pair_created.pair)
      LEFT JOIN `xed-project-237404.footprint_etl.bsc_erc20_tokens` t ON t.contract_address = pair_created.token1
    LEFT JOIN
      `footprint-blockchain-etl.crypto_bsc.transactions` tx
    ON
      tx.hash = event.transaction_hash AND DATE(tx.block_timestamp) {match_date_filter}
