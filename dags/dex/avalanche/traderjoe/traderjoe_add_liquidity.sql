...
FROM
  `footprint-etl.avalanche_traderjoe.JoePair_event_Mint` event
LEFT JOIN
  `footprint-etl.avalanche_traderjoe.JoeFactory_event_PairCreated` pair_created
ON
...
SELECT
  ...
  'AMM' AS type,
  pair_created.token1 AS token_address,
  event.contract_address AS exchange_address,
  event.transaction_hash AS tx_hash,
  DATETIME(event.block_timestamp) AS block_time,
  tx.from_address AS tx_from
