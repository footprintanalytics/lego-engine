...
FROM (
    ...
    *
    ...
    DISTINCT tokenId
    FROM
    `footprint-etl.Ethereum_izumi_autoparse.Deposit_event_addressIndexed_uint256_uint256` ) ) transfer
    LEFT JOIN
    ...
    *
    ...
    DISTINCT tokenId
    FROM
    `footprint-etl.Polygon_izumi_autoparse.Deposit_event_addressIndexed_uint256_uint256` ) ) transfer
    LEFT JOIN
    `footprint-etl.Polygon_izumi_autoparse.Deposit_event_addressIndexed_uint256_uint256` deposit
    ON
    transfer.tokenId = deposit.tokenId )) owner
    INNER JOIN
    day_list d
ON
