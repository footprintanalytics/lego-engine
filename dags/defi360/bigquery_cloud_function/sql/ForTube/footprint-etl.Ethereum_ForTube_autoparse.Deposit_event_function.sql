CREATE OR REPLACE FUNCTION
    `footprint-etl.Ethereum_ForTube_autoparse.Deposit_event_function`(data STRING)
    RETURNS STRUCT<
    `user_address` STRING,
    `token_address` STRING,
    `cheque_token_address` STRING,
    `amount_deposited` STRING,
    `underlying_deposited` STRING,
    ...
    var parsedEvent = {
        ...
        "inputs": [
            ...
            {"indexed": false, "internalType": "uint256", "name": "amount_deposited", "type": "uint256"},
            {"indexed": false, "internalType": "uint256", "name": "underlying_deposited", "type": "uint256"},
