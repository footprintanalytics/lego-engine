...
class TokenBalancesFortube(ContractAdapter):
    # 自己确定数据集

    # time_partitioning_field = "day"
    task_name = "ud_token_balances_fortube"
    time_partitioning_field = None
    execution_time = "35 2,6 * * *"
    history_date = '2022-02-28'
    schema_name = "defi360/schema/token_balances.json"
    depend_sql_path = 'defi360/contracts_adapter/token_balances/fortube/depend_data.sql'
    history_day = 90
...
if __name__ == '__main__':
    job = TokenBalancesFortube()
    job.project_id_test = "gaia-data"
    job.data_set_test = "origin_data"
    #
    # # 跑当天数据
    job.load_daily_data(run_date="2022-03-31", debug=False)
