...
# todo 目前仅支持izumi
class PoolHoldersData(ContractAdapter):
    # prod
    project_id = 'gaia-data'
    # 自己确定数据集
    data_set = "gaia"
    task_name = "pool_holders_gaia_data"
    time_partitioning_field = "day"
    execution_time = "50 3 * * *"
    schema_name = "defi360/schema/pool_holders.json"
    abi = "defi360/contracts_adapter/abi/izumi.json"
    depend_sql_path = 'defi360/contracts_adapter/pool_holders_gaia_data/depend_data.sql'
    history_date = '2022-04-05'
    # test
    project_id_test = 'gaia-data'
    data_set_test = 'gaia'
    pool_data = [
        {
            "chain": "Ethereum",
            ...
            "chain": "Arbitrum",
            "pool_address": "0xb2decea19d58ebe10ab215a04db2edbe52e37fa4",
            "nft_address": "0xc36442b4a4522e871399cd717abdd847ab11fe88"
        }
    ]

    def get_bigquery_table_name(self):
        table_name = "pool_holders"
    ...
    def get_depend_data(self, run_date):
        ...
        for chain in chains:
            ...
            sql_list.append(_sql.format(chain=chain, chain_lower=str(chain).lower()))
        try:
...
if __name__ == '__main__':
    ...
    #
    # # 跑当天数据
    # poolHolders.load_daily_data(debug=False)

    # 跑历史数据
    # poolHolders.load_history_data(debug=False)
