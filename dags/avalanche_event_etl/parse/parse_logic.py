...
from utils.bigquery_utils import submit_bigquery_job, read_bigquery_schema_from_json_recursive, query, create_view, does_table_exist
from avalanche_event_etl.parse.templates import render_parse_udf_template, render_parse_sql_template, render_merge_template, render_stitch_view_template
...
def parse(
    ...
    if not ds:
        ds = DateUtil.utc_start_of_date(DateUtil.utc_x_hours_ago(24)).strftime('%Y-%m-%d')

    internal_project_id = destination_project_id + '-internal'
    dataset_name = 'avalanche_' + table_definition['table']['dataset_name']
...
def create_or_replace_internal_view(
    ...
    print('create_or_replace_internal_view: ' + sql)
...
def create_or_update_history_table(
    ...
    print('create_or_update_history_table ====>>>>', parse_all_partitions)
    print('create_or_update_history_table:', sql)
    query(bigquery_client, sql, destination=temp_table_ref)

    # # # Copy / merge to destination

    print('create_or_update_history_table ====>>>>>> parse_all_partitions', parse_all_partitions)
    if parse_all_partitions:
        ...
        print('copy_job ====== copy_job ======== temp_table_ref', temp_table_ref)
        print('copy_job ====== copy_job ======== dest_table_ref', dest_table_ref)
        ...
        print('merge_sql ====== merge_sql ======== temp_table_ref', merge_sql)
...
def create_or_replace_stitch_view(
    ...
    dest_view_ref = dataset.table(table_name)
    print('create_or_replace_stitch_view:', sql)
...
def get_source_table(
    ...
    partitioned_dataset_name = 'crypto_avalanche'
    ...
    print('get_source_table parse_mode: ===>>>', parse_mode)
    print('get_source_table source_project_id: ===>>>', source_project_id)
    print('get_source_table source_dataset_name: ===>>>', source_dataset_name)
    print('get_source_table source_table_name ===>>>', source_table_name)
