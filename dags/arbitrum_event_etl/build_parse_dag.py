...
from arbitrum_event_etl.parse.parse_logic import ref_regex, parse, create_dataset
...
def build_parse_dag(
    ...
    SOURCE_DATASET_NAME = 'crypto_arbitrum'
    ...
    # This prevents failing all dags as they are constructed in a loop in arbitrum_parse_dag.py
    ...
    def create_parse_task(table_definition):
        ...
        def parse_task(ds, **kwargs):
            ...
            if _ds:
                ds = _ds
            print('arbitrum parse_task 本次执行获取的ds为', ds)
            parse(
                ...
                sqls_folder=os.path.join(PROJECT_PATH, 'resources/stages/parse/arbitrum_sqls'),
    ...
    wait_for_arbitrum_load_dag_task = BashOperator(
    ...
    for json_file in json_files:
        ...
        wait_for_arbitrum_load_dag_task >> task
    ...
    full_dataset_name = 'arbitrum_' + dataset_name
    ...
    if notification_emails and len(notification_emails) > 0 and send_success_email:
        send_email_task = EmailOperator(
            ...
            subject='ARBITRUM ETL Airflow Parse DAG Succeeded',
            html_content='ARBITRUM ETL Airflow Parse DAG Succeeded for {}'.format(dag_id),
            dag=dag
        )
