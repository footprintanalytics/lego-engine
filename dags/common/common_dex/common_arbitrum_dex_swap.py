...
class CommonArbitrumDexSwap(CommonDexSwap):
    ...
    def build_origin_source_sql(self, match_date_filter: str):
        ...
        return """
            ...
            inner join `footprint-blockchain-etl.crypto_arbitrum.transactions` tx
            on dexs.tx_hash = tx.hash and DATE(tx.block_timestamp) {match_date_filter}
            left join `xed-project-237404.footprint_etl.arbitrum_erc20_tokens` tokena
            on Lower(tokena.contract_address) = Lower(dexs.token_a_address)
            left join `xed-project-237404.footprint_etl.arbitrum_erc20_tokens` tokenb
