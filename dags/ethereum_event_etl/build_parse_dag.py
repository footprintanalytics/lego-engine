...
from utils.bigquery_utils import create_view, share_dataset_all_users_read
from utils.common import read_json_file, read_file
from ethereum_event_etl.parse.parse_logic import ref_regex, parse, create_dataset
...
bigquery_client = bigquery.Client(project='footprint-etl-internal')
...
def build_parse_dag(
    ...
    SOURCE_PROJECT_ID = 'footprint-blockchain-etl'
    ...
    def create_parse_task(table_definition):
        ...
        def parse_task(ds, **kwargs):
            ...
            _ds = pydash.get(conf, 'ds')
            if _ds:
                ds = _ds
            print('ethereum parse_task 本次执行获取的ds为', ds)
            parse(
    ...
    def create_add_view_task(dataset_name, view_name, sql):
        def create_view_task(ds, **kwargs):
            ...
            dest_table_ref = create_dataset(bigquery_client, dataset_name, parse_destination_dataset_project_id).table(dest_table_name)
    ...
    def create_share_dataset_task(dataset_name):
        def share_dataset_task(**kwargs):
            if parse_destination_dataset_project_id != 'blockchain-etl':
                ...
                share_dataset_all_users_read(bigquery_client, f'{parse_destination_dataset_project_id}.{dataset_name}')
                share_dataset_all_users_read(bigquery_client, f'{parse_destination_dataset_project_id}-internal.{dataset_name}')
    ...
    wait_for_ethereum_load_dag_task = BashOperator(
        task_id='partition_done',
        bash_command='echo parse starting',
        dag=dag
    )

    parse_ethereum_dag_start_task = PythonOperator(
        task_id='parse_start',
        python_callable=parse_ethereum_start,
        provide_context=True,
        execution_timeout=timedelta(minutes=60),
        dag=dag
    )

    parse_ethereum_dag_start_task >> wait_for_ethereum_load_dag_task

    parse_ethereum_dag_end_task = PythonOperator(
        task_id='parse_end',
        python_callable=parse_ethereum_end,
        provide_context=True,
        execution_timeout=timedelta(minutes=60),
        dag=dag
    )
    ...
    checkpoint_task >> share_dataset_task >> parse_ethereum_dag_end_task
...
def task_on_failure_callback(context):
    ...
    message = f'[Airflow] DAG {dag_id} - {task_id}: Fail, 影响范围: ethereum-parse解析中 {dag_id} 平台解析事件数据缺失'
    send_to_slack(message)


def parse_ethereum_start(**kwargs):
    try:
        conf = kwargs.get('dag_run').conf
        ds = pydash.get(conf, 'ds')
        dag = kwargs.get('dag')
        dag_id = dag.dag_id
        """任务开始的操作"""
        rule_name = Constant.DASH_BOARD_RULE_NAME['TASK_EXECUTION']
        desc = '{task_name} rule_name is task execution'.format(task_name=dag_id)
        desc_cn = Constant.DASH_BOARD_RULE_NAME_DESC_CN['TASK_EXECUTION']
        item_value = 1
        result_code = Constant.DASH_BOARD_RESULT_CODE['EXCEPTION']
        sql = ''
        save_monitor(dag_id, rule_name, item_value, result_code, desc, desc_cn, sql, ds)
    except Exception as e:
        print('任务执行开始检查失败')


def parse_ethereum_end(**kwargs):
    try:
        conf = kwargs.get('dag_run').conf
        ds = pydash.get(conf, 'ds')
        dag = kwargs.get('dag')
...
def save_monitor(dag_id: str, rule_name: str, item_value: int, result_code: int, desc: str, desc_cn: str, sql: str, ds: str = None):

    stats_date = DateUtil.utc_start_of_date(DateUtil.utc_x_hours_ago(24 * 1))
    query = {
        'task_name': dag_id,
        'rule_name': rule_name,
        'field': '',
