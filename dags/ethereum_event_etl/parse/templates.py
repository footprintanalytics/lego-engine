...
def render_merge_template(
    ...
    template = get_merge_table_sql_template(parser_type, sqls_folder)
...
def get_merge_table_sql_template(parser_type, sqls_folder):
    if parser_type == 'log':
        filename = 'merge_logs_table.sql'
    else:
        filename = 'merge_traces_table.sql'
    filepath = os.path.join(sqls_folder, filename)
    with open(filepath) as file_handle:
        content = file_handle.read()
        return content
