...
from ethereum_event_etl.parse.templates import render_parse_udf_template, render_parse_sql_template, render_merge_template, render_stitch_view_template
...
def parse(
    ...
    if not ds:
        ds = DateUtil.utc_start_of_date(DateUtil.utc_x_hours_ago(24 * 1)).strftime('%Y-%m-%d')
...
def create_or_replace_internal_view(
    ...
    if parser_type == 'trace':
        ds = DateUtil.utc_start_of_date(DateUtil.utc_x_hours_ago(24 * 2)).strftime('%Y-%m-%d')
...
def get_source_table(
    ...
    # 这里获取source table 的规则: logs 使用 自建 traces 使用blockchain-etl 这里traces 不允许指定日期重刷
    if parse_mode == ParseMode.HISTORY_ALL_DATES:
        ...
        if parser_type == 'log':
            ...
            source_project_id = 'blockchain-etl-internal'
